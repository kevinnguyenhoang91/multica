    ...
    entries: [
      {
        version: "0.2.20",
        date: "2026-04-29",
        title: "Quick Create Issue、Agent Presence v3 与 Daemon WebSocket 心跳",
        changes: [],
        features: [
          "Quick Create Issue —— 在任意页面按 `c` 输入一句话并选 Agent，即可异步创建 Issue，结果回执送达 Inbox",
          "Agent Presence v3 —— 可用性与最近任务拆成更清晰的两条信号；Issue 详情右侧新增 Execution Log，可看到当前 active run 与历史 run",
          "Daemon ↔ Server 心跳改走 WebSocket，HTTP 自动 fallback，任务起跑延迟更低",
          "Mention 选择器按本机最近使用排序，常用 Agent / 同事会更早出现",
          "全新的 Editorial 404 页面取代原本的简陋 not-found",
          "后端支持通过 `MULTICA_CLAUDE_ARGS` / `MULTICA_CODEX_ARGS` 配置 Agent CLI 默认参数",
        ],
        improvements: [
          "Manual 与 Agent 创建 Issue 现在共享同一个 Dialog 外壳，picker Agent 会被默认设为 assignee",
          "Server 用 Redis 缓存 PAT / Daemon Token 校验（10 分钟 TTL），大型团队不再让 DB 抗下每次请求",
          "心跳鉴权耗时拆成 decode / runtime lookup / workspace check，慢请求一眼可见",
          "Quick Create UX 打磨：类型标签更清晰、暗色 toast、更友好的占位文案",
          "桌面端切换到主题化的 Toaster 包装，颜色与应用风格一致",
          "CLI / Agent 文档补齐 label、subscriber 与 create / update 的完整参数说明",
        ],
        fixes: [
          "Quick Create 不再卡住 queued 任务、不再因附件上传失败而重复创建 Issue；对话框高度受控且长内容可滚动",
          "Agent 任务完成评论保留换行，不再渲染成字面量 `\\n`",
          "Agent 回复评论统一走 stdin，多行内容的格式被完整保留",
          "Assignment 触发的 Agent run 强制先读 comment history",
          "Agent 自身发出的根评论不再继承父评论的 @mention，避免互相唤起的死循环",
          "Codex 每任务 config 会剥离 `[[skills.config]]`，并默认关闭 native multi-agent",
          "Windows 下 Cursor Agent 走 PowerShell `-File` 启动，保留多行 prompt",
          "Popover 点击冒泡与 Agent inspector 的 presence loading 已修复",
          "Chat V2 临时回滚为右下角悬浮抽屉，便于在迭代期持续打磨",
        ],
      },
      {
        version: "0.2.19",
