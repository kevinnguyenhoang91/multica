    ...
    entries: [
      {
        version: "0.2.20",
        date: "2026-04-29",
        title: "Quick Create Issue, Agent Presence v3 & Daemon WebSocket Heartbeat",
        changes: [],
        features: [
          "Quick Create Issue — press `c` to file an issue with one line plus an agent; creation runs async and the result lands in your inbox",
          ...
          "Mention picker ranks suggestions by your local recency, so the people and agents you actually mention surface first",
          "Editorial 404 page replaces the bare not-found screen",
        ...
        improvements: [
          ...
          "Server caches PAT / daemon token lookups in Redis (10m TTL) — large fleets stop hammering the database on every request",
          "Heartbeat auth timing broken into decode / runtime lookup / workspace check, so slow probes are obvious in logs",
          "Quick-create polish — clearer kind labeling, dark-mode toast, better empty placeholder",
          "Desktop switches to the themed Toaster wrapper instead of bare sonner, matching app theming",
          "CLI / agent docs cover label, subscriber, and the full create / update flag set",
        ],
        fixes: [
          "Quick-create no longer leaves tasks stuck queued, no longer duplicates the issue when an attachment upload fails, and the dialog now bounds height and scrolls long content",
          "Agent task-completion comments respect newlines instead of rendering literal `\\n`",
          "Agent reply path uses stdin for formatted comments, so multi-line content keeps its formatting",
          "Assignment-triggered agent runs always read comment history first",
          "Agent-authored root comments no longer inherit parent @mentions, breaking accidental agent loops",
          "Codex per-task config strips `[[skills.config]]` and disables native multi-agent by default",
          "Cursor agent on Windows preserves multi-line prompts via PowerShell `-File`",
          "Popover click bubbling and presence loading on the agent inspector are fixed",
          "Chat V2 reverts to the right-bottom floating drawer while the page is iterated",
        ],
      },
      {
        version: "0.2.19",
