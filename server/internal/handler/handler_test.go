...
func TestUpdateIssueInReviewTransitionReassignsToCreator(t *testing.T) {
	ctx := context.Background()
	var agentID string
	...
	if updated.AssigneeType == nil || *updated.AssigneeType != "member" {
		t.Fatalf("expected assignee_type=member (creator), got %v", updated.AssigneeType)
	}
	if updated.AssigneeID == nil || *updated.AssigneeID != testUserID {
		t.Fatalf("expected assignee_id=%s (creator), got %v", testUserID, updated.AssigneeID)
	}
}

// TestUpdateIssueInReviewTransitionRejectsArchivedCreatorAgentAssignee is the
// regression test for KHA-53: a status-only update to in_review should NOT
// bypass validateAssigneePair when the in_review handoff derives an archived
// creator agent as the new assignee.
func TestUpdateIssueInReviewTransitionRejectsArchivedCreatorAgentAssignee(t *testing.T) {
	ctx := context.Background()

	// Create a fresh agent that will act as the issue creator.
	creatorAgent := createHandlerTestAgent(t, "KHA53 Creator Agent", []byte("[]"))

	// Create the issue normally (creator = testUserID/member).
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":  "kha-53 archived-creator regression",
		"status": "in_progress",
	})
	testHandler.CreateIssue(w, req)
	if w.Code != http.StatusCreated {
	...
	json.NewDecoder(w.Body).Decode(&created)
	t.Cleanup(func() {
		r := newRequest("DELETE", "/api/issues/"+created.ID, nil)
		r = withURLParam(r, "id", created.ID)
		testHandler.DeleteIssue(httptest.NewRecorder(), r)
	})

	// Override creator to the fresh agent directly in DB.
	if _, err := testPool.Exec(ctx,
		`UPDATE issue SET creator_type = 'agent', creator_id = $1 WHERE id = $2`,
		creatorAgent, created.ID,
	); err != nil {
		t.Fatalf("override creator: %v", err)
	}
	...
	// Status-only in_review update — must be rejected because handoff would
	// derive the archived agent as the new assignee.
	w = httptest.NewRecorder()
	req = newRequest("PUT", "/api/issues/"+created.ID, map[string]any{
		"status": "in_review",
	})
	...
	testHandler.UpdateIssue(w, req)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("UpdateIssue: expected 400 (archived creator), got %d: %s", w.Code, w.Body.String())
	}

	// Confirm status was not changed.
	gw := httptest.NewRecorder()
	gr := newRequest("GET", "/api/issues/"+created.ID, nil)
	gr = withURLParam(gr, "id", created.ID)
	testHandler.GetIssue(gw, gr)
	var got IssueResponse
	json.NewDecoder(gw.Body).Decode(&got)
	if got.Status != "in_progress" {
		t.Fatalf("expected status=in_progress (unchanged), got %q", got.Status)
	}
}
